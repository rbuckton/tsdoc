...
const TSDocConfiguration = makeRelative(path.dirname(resolvedSpecTsFile), '../src/configuration/TSDocConfiguration');
...
const lines = [
    ...
    `import { TSDocConfiguration } from ${JSON.stringify(TSDocConfiguration)};`,
    ``,
    'expect.addSnapshotSerializer(SnapshotSerializer.serializer);',
    ``,
    `function parse(text: string): Document {`,
    `    const parser: BlockParser = new BlockParser(new TSDocConfiguration(), text, undefined, ${gfm});`,
    `    return parser.parse();`,
    `}`,
    ``,
